...
class AbstractSceneMap:
	...
	def recordXml( self, xml ):
		"""
			\remarks	define a way to record this map to xml
		...
		xml.setAttribute( 'name', 	self.mapName() )
		xml.setAttribute( 'id', 	self.mapId() )
		return True
	...
	def fromXml( scene, xml ):
		...
		"""
		if ( xml ):
			return scene.findMap( mapName = xml.attribute( 'name' ), mapId = int(xml.attribute( 'id', 0 )) )
		return None
